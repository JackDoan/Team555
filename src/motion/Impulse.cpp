...
bool Impulse::run(GameState& gs) {
    ...
    switch(impulseState) {
        case IMPULSEDONE:
            ...
            break;
        case STAGING1:
            if (!Motion::stillStaging(gs.mallet.location, impulseVector[0])) {
            ...
            break;
        case STAGING2:
            if (!Motion::stillStaging(gs.mallet.location, impulseVector[1])) {
            ...
            break;
        case HITTING:
            if (!Motion::stillStaging(gs.mallet.location, impulseVector[2])) {
...
std::vector<cv::Point_<int>> Impulse::findImpulseVector(GameState& gs) {
